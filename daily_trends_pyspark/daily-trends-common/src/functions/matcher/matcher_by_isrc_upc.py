...
class MatcherByIsrcUpc:
    def __init__(self, reference: DataFrame):
        self.reference = reference

    def product_reference(self, table: DataFrame) -> DataFrame:
        # Preprocess table to prevent duplicates (same ISRC but distinct validity periods)
        df = self.reference.join(
            table.select(ProductMatchingSchema.ISRC_MATCHING, ProductMatchingSchema.UPC_MATCHING, ProductMatchingSchema.DATE_MATCHING),
            self.join_condition(),
            "left_semi"
        )
        window = Window.partitionBy(F.col(ProductReferenceSchema.ISRC), F.col(ProductReferenceSchema.UPC)).orderBy(F.col(ProductReferenceSchema.ID_TRACK).desc_nulls_last())

        # Keep only the ISRC with the max ID_TRACK
    ...
    def join_condition(self):
        ...
        return (F.col(ProductReferenceSchema.ISRC) == F.col(ProductMatchingSchema.ISRC_MATCHING)) & \
            (F.col(ProductReferenceSchema.UPC) == F.col(ProductMatchingSchema.UPC_MATCHING)) & \
