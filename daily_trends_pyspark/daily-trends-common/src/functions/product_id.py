...
class ProductId:
    ...
    def add_product_informations(self, df: DataFrame, destination: str, product_id_columns=None) -> DataFrame:
        ...
        for idx, matching in enumerate(self.matchers):
            if idx == 0:
                df = self.first_product_matching(df, destination, matching, product_id_columns)
            else:
        ...
        df = df.drop("TRACK_STORE_ID_MATCHING", "ISRC_MATCHING", "UPC_MATCHING", "MATCHING_ALGORITHM") \
    ...
    def first_product_matching(self, df: DataFrame, destination: str, matching, product_id_columns):
        ...
        if matching.name() == "MatcherByTrackStoreId":
            df = df.join(matching.product_reference(None), matching.join_condition(), "left_outer") \
                .withColumn(
                "MATCHING_ALGORITHM",
            ...
            for c in product_id_columns.extra_product_keys:
                ...
                df = df.withColumn(product_id_columns.main_product_key.source_name, lit(None).cast(product_id_columns.main_product_key.data_type)) \
